...
3. If the release includes user-facing changes, add an entry to `app/src/constants/changelog.ts` (`CHANGELOG` array) for the new version — a `date` and a short list of `items`. This is what powers the in-app "what's new" screen shown to users after they update; a version with no matching entry just won't show anything, silently.
4. From `app/`, run `npm run release` (this runs `tsc -b && vite build`, then zips `app/build` into `chrome-webstore/releases/<name>-v<version>.zip`).
5. Verify the new zip exists in `chrome-webstore/releases/` with the expected version number.
6. If the release includes user-facing feature changes, ask whether `chrome-webstore/description.md` and/or `chrome-webstore/testing-instructions.md` need updating to reflect them (testing instructions must stay ≤ 500 characters, description ≤ 16,000 characters, both plain text since those are the actual Chrome Web Store field limits).
7. Commit the version bump, changelog entry, and new release zip, following this repo's commit style (short, lowercase, imperative — e.g. `v3.1.0`), then ask before pushing.
8. Remind the user that publishing the new zip to the Chrome Web Store is a manual step through the developer dashboard — this skill does not do that part.
